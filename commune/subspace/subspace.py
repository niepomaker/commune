...
class Subspace(c.Module):
    ...
    def get_stake_to( self, key: str = None, module_key=None, block: Optional[int] = None, netuid:int = None , fmt='j' , network=None, update=False, **kwargs) -> Optional['Balance']:
        ...
        key_address = self.resolve_key_ss58( key )
        netuid = self.resolve_netuid( netuid )
        stake_to =  {k: self.format_amount(v, fmt=fmt) for k, v in self.query( 'StakeTo', params=[netuid, key_address], block=block, update=update, **kwargs )}
    ...
    def my_value(self, network = None,fmt=fmt, decimals=2, update=False):
    ...
    def my_total_stake(self,  network = None,  fmt=fmt, decimals=2, update=False):
        ...
        for netuid in self.netuids(network=network):
            my_total_stake +=  c.asubmit(self.my_stake, kwargs=dict(network=network, netuid=netuid, fmt=fmt, decimals=decimals, update=update).values())
